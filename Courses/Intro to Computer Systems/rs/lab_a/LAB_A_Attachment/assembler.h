...
enum StringType { sComment, sLabel, sValue, sOpcode, sOperand, sError };
...
inline void SetDebugMode(bool debug) { gIsDebugMode = debug; }
inline void SetErrorLogMode(bool error) { gIsErrorLogMode = error; }
inline void SetHexMode(bool hex) { gIsHexMode = hex; }
...
inline int RecognizeNumberValue(const std::string &str) {
  int val = 0;
  switch (str[0]) {
    case 'B':
      return std::stoi(str.substr(1), nullptr, 2);
    case '#':
      return std::stoi(str.substr(1), nullptr);
    case 'X':
...
inline std::string NumberToAssemble(int number) {
  return std::bitset<16>(number).to_string();
}

inline std::string NumberToAssemble(const std::string &number) {
  return NumberToAssemble(RecognizeNumberValue(number));
}

inline std::string ConvertBin2Hex(const std::string &bin) {
  std::string hex;
  for (int i = 0; i < bin.size(); i += 4) {
    int num = 0;
    for (int j = 0; j < 4; ++j) num += bin[i + j] - '0' << (3 - j);
    hex += DecToChar(num);
  }
  return hex;
}
...
class assembler {
 private:
  ...
  std::string TranslateOperand(int current_address, const std::string &str,
                               int opcode_length = 3);

 public:
  int assemble(const std::string &input_filename, std::string &output_filename);
};